...
func NewFileServ(t_ask *http.Request,t_res *http.ResponseWriter,t_local string){
	...
	L2I("NewFileServ new file dowload:"+urlPath)
	ix := strings.LastIndex(urlPath,"/");if ix==-1 {
		L3E("NewFileServ strings.LastIndex find prefix %s %d",urlPath,ix)
		return
	}
	prefix := urlPath[:ix+1]
	...
	ix = strings.LastIndex(strx,"/");if ix==-1 {
		L3E("NewFileServ strings.LastIndex find folder id %s %d",urlPath,ix)
		return
	}
	folderid := strx[ix+1:]

	dirx := t_local + GetOSSeptor()+folderid
	L2I("NewFileServ start fileservr(" + prefix + " " + dirx + ")")
